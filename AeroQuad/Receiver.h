...
public:
  ...
  int transmitterCommand[7];                     // Roll, pitch, yaw, throttle, mode, arm/disarm, calibrate == 0, 1, 2, 3, 4, 5, 6)
  ...
  Receiver(void) {
    transmitterCommand[0] = 1500;
    transmitterCommand[1] = 1500;
    transmitterCommand[2] = 1500;
    transmitterCommand[3] = 1000;
    transmitterCommand[4] = 1;       // acrobat to start with: 1 = Acrobat, 0 = stable
    transmitterCommand[5] = 0;       // 0 = disarm
    transmitterCommand[6] = 0;       // 1 = caliberate
  ...
  const int getRaw(byte channel) {
   ...
   void setRaw(byte channel, int val) {
     transmitterCommand[channel] = val;
  }
...
int bluetoothData[7];
//flag = R
void receiveRoll(byte flag, byte numOfValues)
{
    bluetoothData[0] = meetAndroid.getInt();
}

//flag = P
void receivePitch(byte flag, byte numOfValues)
{
    bluetoothData[1] = meetAndroid.getInt();
}

//flag = Y
void receiveYaw(byte flag, byte numOfValues)
{
    bluetoothData[2] = meetAndroid.getInt();
    //Serial.print("Yaw: ");
    //Serial.print(bluetoothData[2]);
    //Serial.print("\n");
}

//flag = Y
void receiveThrottle(byte flag, byte numOfValues)
{
    bluetoothData[3] = meetAndroid.getInt();
}

//flag = Y
void receiveMode(byte flag, byte numOfValues)
{
    bluetoothData[4] = meetAndroid.getInt();
}


//flag = Y
void receiveArm(byte flag, byte numOfValues)
{
    bluetoothData[5] = 1;
}

//flag = Y
void receiveDisarm(byte flag, byte numOfValues)
{
    bluetoothData[5] = 0;
}
...
//flag = Y
void receiveCalibrate(byte flag, byte numOfValues)
{
    bluetoothData[6] = 1;
}
...
public:
  void initialize() {
    this->_initialize(); // load in calibration xmitFactor from EEPROM
    bluetoothData[0] = 1500;
    bluetoothData[1] = 1500;
    bluetoothData[2] = 1500;
    bluetoothData[3] = 1000;
    bluetoothData[4] = 1;
    bluetoothData[5] = 0;
    bluetoothData[6] = 0;
  }
  ...
  void read(void) 
  {
     transmitterCommand[0] = bluetoothData[0];
     transmitterCommand[1] = bluetoothData[1];
     transmitterCommand[2] = bluetoothData[2];
     transmitterCommand[3] = bluetoothData[3];
     transmitterCommand[4] = bluetoothData[4];
     transmitterCommand[5] = bluetoothData[5];
     transmitterCommand[6] = bluetoothData[6];
  }
};
