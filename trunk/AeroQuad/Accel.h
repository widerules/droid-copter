...
public:
  Accel_AeroQuadMega_v2() : Accel(){
    accelAddress = 0x53; // page 54 and 61 of datasheet
    // Accelerometer value if BMA180 setup for 1.0G
    // Page 27 of datasheet = 0.00013g/LSB, page 3 in ADXL sheet, +- 2g
    accelScaleFactor = 0.039;
  }
  ...
  void initialize(void) {
    ...
    // To initialize and power on. From ADXL345.c, no reset needed
    acc.init();
	
    // updateRegisterI2C(accelAddress, 0x10, 0xB6); //reset device
    // delay(10);  //sleep 10 ms after reset (page 25)

    // Range setting to 00000b, corrs to +-2g. 0 at reset, so not needed.
    acc.setRangeSetting(2);       // check adxl.c
	
     // Set rate BW or LPF to 10Hz. 10Hz is not available, so use 12.5 HZ in normal power mode
     acc.setRate(12.5);                   // pg 16
  ...
  {
    int rawData[3];
    acc.readAccel(&rawData[0], &rawData[1], &rawData[2]);    // 1, 2, 3 correspond to pitch, roll and z axis
  ...
  //Removed "getRaw >> 4". Didnt understand why it was there
  ...
  void calibrate(void) 
  {  
    int findRollZero[FINDZERO];
    int findPitchZero[FINDZERO];
    int findZaxisZero[FINDZERO];
 
    for (int i=0; i<FINDZERO; i++) 
    {
      int data[3];
      acc.readAccel(&data[0], &data[1], &data[2]);   
      findRollZero[i] = data[0];
      findPitchZero[i] = data[1];
      findZaxisZero[i] = data[2];	
      delay(1);
    }
    accelZero[0] = findMode(findRollZero, FINDZERO);
    accelZero[1] = findMode(findPitchZero, FINDZERO);
    accelZero[2] = findMode(findZaxisZero, FINDZERO);
